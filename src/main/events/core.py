...
def check_requirements(window, requirements=None):
    ...
    checks = []
    for mechanism, limit_key in [('radiation','max_radiation_overlap_minutes'), ('geomagnetic','max_geomagnetic_overlap_minutes'), ('communications','max_communications_overlap_minutes'), ('tracked_debris','max_tracked_debris_overlap_minutes')]:
        if limit_key not in requirements: continue
        f=factors.get(mechanism, {}); value=f.get('known_interval_overlap_minutes')
        checks.append({'requirement':limit_key,'mechanism':mechanism,'limit':requirements[limit_key],'value':value,'status':'unknown' if value is None else ('pass' if value <= requirements[limit_key] else 'fail')})
    min_cov=requirements.get('min_data_coverage_fraction')
    if min_cov is not None:
        vals=[f.get('coverage_fraction') for f in factors.values()]; value=min(vals) if vals else None
        checks.append({'requirement':'min_data_coverage_fraction','limit':min_cov,'value':value,'status':'unknown' if value is None else ('pass' if value>=min_cov else 'fail')})
    return {'passed': bool(checks) and all(x['status']=='pass' for x in checks), 'status':'unknown' if any(x['status']=='unknown' for x in checks) else ('pass' if all(x['status']=='pass' for x in checks) else 'fail'), 'checks':checks}

def select_best_window(windows, requirements=None):
    ranked=[]
    for i,w in enumerate(windows):
        req=check_requirements(w, requirements); burden=sum(f.get('known_interval_overlap_minutes',0) for f in w.get('factors',{}).values()); coverage=min((f.get('coverage_fraction',0) for f in w.get('factors',{}).values()), default=0)
        ranked.append((req['status']=='pass', req['status']=='unknown', burden, -coverage, i, req))
    eligible=[x for x in ranked if x[0]] or ([x for x in ranked if x[1]] if ranked else [])
    chosen=min(eligible, key=lambda x:(x[2],x[3],x[4])) if eligible else None
    return {'preferred_window_index':chosen[4] if chosen else None,'reason':'минимальное суммарное пересечение при выполнении требований' if chosen and chosen[0] else 'оснований для безопасного выбора недостаточно','ranked': [{'window_index':x[4],'requirements':x[5],'burden_minutes':x[2],'coverage_fraction':-x[3]} for x in sorted(ranked,key=lambda x:(not x[0],not x[1],x[2],x[3]))]}
...
def assess(bundle, q):
    ...
    if cutoff:
        ...
        for e in events:
            ...
            if not e.get('published_at'):
                excluded += 1; excluded_unknown_publication += 1; continue
    ...
    return dict(algorithm_version=VERSION, query=q, generated_at=iso(now()),
                ...
                sources=bundle['sources'], exclusions_after_cutoff=excluded, excluded_unknown_publication=excluded_unknown_publication,
