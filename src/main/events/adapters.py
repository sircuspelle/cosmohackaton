...
from src.main.events.core import dt, event, iso, number

DONKI_KINDS = {
    "FLR": ("solar_flare", "communications", "flrID", "beginTime"),
    ...
    for r in records(data):
        if kind == "WSAEnlilSimulations":
            ...
            hours = (
                ...
                if r.get("estimatedDuration") is not None
                else None
            )
            ...
            out.append(
                event(
                    ...
                    "geomagnetic",
                    start,
                    start + timedelta(hours=hours) if hours is not None else None,
                    src,
                    r,
                    basis="external_forecast",
                    temporal="interval" if hours is not None else "open",
                    values={
                        "estimated_duration_hours": hours,
                        "model_completed_at": r.get("modelCompletionTime"),
                        "arrival_uncertainty_hours": None,
                    },
                    related=[
                        x["cmeid"] for x in r.get("cmeInputs") or [] if x.get("cmeid")
                    ],
                    rule="Earth ENLIL arrival plus model duration; no local ISS dose conversion",
                    limitations=[
                        "Model completion is not publication time.",
        ...
        if kind == "IPS" and r.get("location") != "Earth":
            continue
        related = [
            x["activityID"] for x in r.get("linkedEvents") or [] if x.get("activityID")
        ]
        instruments = [x.get("displayName", "") for x in r.get("instruments") or []]
        basis = (
            "external_forecast"
            if instruments and all(x.startswith("MODEL:") for x in instruments)
            else "observation"
        )
        relevance = (
            "context_only"
        ...
        out.append(
            event(
                ...
                relevance=relevance,
                temporal=temporal,
                published=r.get("submissionTime"),
                values=values,
                related=related,
                rule="Provider event catalog; end is unknown unless explicitly given",
                limitations=[
                    "Catalog completeness is not continuous sensor coverage.",
...
def kp(data, src, config):
    ...
    for r in records(data):
        ...
        basis = {
            ...
            "estimated": "external_estimate",
            "predicted": "external_forecast",
...
def noaa_date(text, labels):
    for label in labels:
        m = re.search(
            label + r":\s*(\d{4} [A-Za-z]{3} \d{1,2} \d{4}) UTC", text, re.IGNORECASE
        )
        if m:
...
def alerts(data, src, config):
    ...
    for r in records(data):
        ...
        start = noaa_date(message, ["Valid From", "Threshold Reached", "Begin Time"])
            ...
            message, ["Now Valid Until", "Valid Until", "Valid To", "End Time"]
        )
        context.append(
            dict(
                kind="noaa_bulletin",
                ...
                declared_start=iso(start) if start else None,
                declared_end=iso(end) if end else None,
                lifecycle="cancellation"
                if re.search(r"CANCEL|ENDED", message, re.IGNORECASE)
...
def socrates(data, src, config):
    ...
    for r in reader:
        ...
        subtype = (
            ...
            if "R/B" in name
            else "payload_or_unknown"
        )
        padding = timedelta(minutes=config["conjunction_buffer_minutes"])
        out.append(
            event(
                "conjunction:"
                + ":".join([r["NORAD_CAT_ID_1"], r["NORAD_CAT_ID_2"], iso(start)]),
                "iss_conjunction",
                "tracked_debris",
                start - padding,
                start + padding,
                src,
                r,
                relevance="iss_conjunction",
                basis="external_forecast",
                values=dict(
                    tca=iso(start),
                    miss_distance_km=distance,
                    relative_speed_km_s=speed,
                    maximum_probability_bound=maximum_probability,
                    object_name=name,
                    object_type=subtype,
                ...
                limitations=[
                    "Review buffer is a configurable planning rule, not timing covariance.",
...
def jpl(data, src, config):
    ...
    for r in rows:
        context.append(
            dict(
                kind="asteroid_earth_approach",
                designation=r["des"],
                time_tdb=r["cd"],
                jd_tdb=number(r["jd"]),
                time_scale="TDB",
                time_uncertainty_3sigma=r["t_sigma_f"],
                distance_au=number(r["dist"]),
                distance_min_3sigma_au=number(r["dist_min"]),
                distance_max_3sigma_au=number(r["dist_max"]),
                relative_speed_km_s=number(r["v_rel"]),
                orbit_version=r["orbit_id"],
                source_url=src["url"],
                snapshot_id=src["snapshot_id"],
...
def goes(data, src, config):
    ...
    threshold = (
        config["proton_threshold_pfu"] if proton else config["xray_threshold_w_m2"]
    )
    ...
    for r in records(data):
        ...
        if flux is not None and (
            flux < 0
            or r.get("electron_contaminaton") is True
            or r.get("electron_contamination") is True
        ):
            flux = None
        samples[t] = (flux, r)
    if not samples:
    ...
    for i, (t, (flux, r)) in enumerate(ordered):
        ...
        contiguous = (
            nxt
            and (nxt[0] - t).total_seconds() <= cadence * 1.5
            and nxt[1][0] is not None
            and nxt[1][1].get("satellite") == r.get("satellite")
        )
        ...
        if contiguous:
            coverage.append(
                dict(
                    ...
                    fetched_at=src["fetched_at"],
                    snapshot_id=src["snapshot_id"],
                    scope="goes_selected_channel_interpolation",
                    basis="observation",
                )
            )
        if flux < threshold:
        ...
        if active is not None and active["end"] == iso(t):
            active["end"] = iso(end)
            active["values"]["maximum_flux"] = max(
                active["values"]["maximum_flux"], flux
            )
        else:
            active = event(
                kind + ":" + iso(t),
                ...
                values={
                    ...
                    "right_censored": not bool(contiguous),
                },
                rule="Threshold segments; previous value held only until adjacent valid sample",
                limitations=[
                    "Threshold episode is not the official NOAA flare/event lifecycle.",
