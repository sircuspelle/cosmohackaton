...
from src.main.events.core import assess, iso, now
        ...
        def do_GET(self):
            ...
            if path == "/requirements":
                return self.reply(
                    ...
                    {
                        "requirements": {
                            "max_radiation_overlap_minutes": "number",
                            "max_geomagnetic_overlap_minutes": "number",
                            "max_communications_overlap_minutes": "number",
                            "max_tracked_debris_overlap_minutes": "number",
                            "min_data_coverage_fraction": "0..1",
                        },
                        "note": "Передаются в поле requirements запроса",
                    },
                )
            if path == "/health":
            ...
            if path in ("/", "/docs"):
                return self.reply(
                    200,
                    {
                        "service": "EVA event metrics",
                        "endpoints": {
                            "POST /assess": {
                                "start": "ISO UTC",
                                "duration_hours": "1..8",
                                "search_hours": "0..24",
                                "step_minutes": "15..1440",
                                "mode": "live|reconstruction|as_of",
                                "as_of": "required for as_of",
                                "force_refresh": False,
                            },
                            "GET /runs/{id}": "Saved result",
                            "GET /snapshots/{id}": "Raw response evidence",
                            "GET /health": "Health",
                        },
                        "note": "Use app.py --help for offline and demo modes",
                    },
                )
            for prefix, method in [("/runs/", "run"), ("/snapshots/", "snapshot")]:
                if path.startswith(prefix):
                    item = getattr(Store(config["database"]), method)(
                        path[len(prefix) :]
                    )
                    return self.reply(
                        200 if item else 404, item or {"error": "not_found"}
                    )
        ...
        def do_POST(self):
            ...
            try:
                ...
                if not 0 < length <= 65536:
                    return self.reply(
                        413, {"error": "Expected JSON body of at most 64 KiB"}
                    )
...
def main():
    ...
    s.add_argument("--duration-hours", type=float, default=6)
    s.add_argument("--search-hours", type=float, default=6)
    ...
    s.add_argument("--force-refresh", action="store_true")
    s.add_argument("--output", default="result.json")
    s = sub.add_parser("conditions")
    ...
    s.add_argument("--output", default="demo_result.json")
    s = sub.add_parser("replay")
    s.add_argument("run_id")
    s.add_argument("--output", default="replayed.json")
    args = p.parse_args()
    ...
    if args.command == "conditions":
        ...
        parse_dt = lambda x: datetime.fromisoformat(
            x.replace("Z", "+00:00")
    ...
    print(
        f"Saved {args.output}: {len(result['windows'])} windows, {len(result['events'])} events; {result['recommendation']['status']}"
    )
